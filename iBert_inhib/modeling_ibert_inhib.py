...
from .configuration_ibert_inhib import IBertConfig
from .quant_modules import IntGELU, IntLayerNorm, IntSoftmax, QuantAct, QuantEmbedding, QuantLinear, SharedQuantLinear
...
class IBertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        # Projection
        mixed_dict = self.qkv(hidden_states, hidden_states_scaling_factor)
        mixed_query_layer, mixed_layer_scaling_factor = mixed_dict["q"]
        mixed_key_layer, _ = mixed_dict["k"]
        mixed_value_layer, _ = mixed_dict["v"]

        # Requantization
        query_layer, query_layer_scaling_factor = self.query_activation(
            mixed_query_layer, mixed_layer_scaling_factor
        )
        key_layer, key_layer_scaling_factor = self.key_activation(mixed_key_layer, mixed_layer_scaling_factor)
        value_layer, value_layer_scaling_factor = self.value_activation(
            mixed_value_layer, mixed_layer_scaling_factor
        )
        ...
        attention_scores = torch.cdist(query_layer, key_layer, p = 1)
        scale = math.sqrt(self.attention_head_size) / self.gamma # fraction gets reversed in line below upon division
        attention_scores = attention_scores / scale
        if self.quant_mode:
            attention_scores_scaling_factor = mixed_layer_scaling_factor / scale
        else:
        ...
        #)
        attention_probs_scaling_factor = scaling_factor = 1 / 2**self.act_bit
