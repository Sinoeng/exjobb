...
class SharedQuantLinear(nn.Module):
        def __init__(
        self, in_features, out_features, bias=True, weight_bit=8, bias_bit=32, per_channel=False, quant_mode=False
            ...
            def __repr__(self):
                self.query.__repr__() + self.key.__repr__() + self.value.__repr__()

            def forward(x, prev_act_scaling_factor=None):
                q_transform = self.query.w.data.detach()
                k_transform = self.key.w.data.detach()
                v_transform = self.value.w.data.detach()

                all_transform = torch.cat(q_transform, k_transform, v_transform)
                w_min = all_transform.min().expand(1)
                w_max = all_transform.max().expand(1)
                shared_scaling_factor = symmetric_linear_quantization_params(self.weight_bit, w_min, w_max)
                return {"q": self.query.forward(x, shared_scaling_factor=shared_scaling_factor, prev_act_scaling_factor=prev_act_scaling_factor),
                        "k": self.key.forward(x, shared_scaling_factor=shared_scaling_factor, prev_act_scaling_factor=prev_act_scaling_factor), 
                        "v": self.value.forward(x, shared_scaling_factor=shared_scaling_factor, prev_act_scaling_factor=prev_act_scaling_factor)
                        }
...
class QuantLinear(nn.Module):
    ...
    def forward(self, x, shared_scaling_factor, prev_act_scaling_factor=None):
        if not self.quant_mode:
        ...
        self.weight_integer = self.weight_function(
            self.weight, self.weight_bit, self.percentile_mode, shared_scaling_factor
        )

        bias_scaling_factor = shared_scaling_factor * prev_act_scaling_factor
